  ...
  static TypeId tid = TypeId ("ns3::Dcf")
    ...
    .AddAttribute ("Aifsn", "The AIFSN: the default value conforms to simple DCA.",
                   ...
                   MakeUintegerAccessor (&Dcf::SetAifsn,
                                         &Dcf::GetAifsn),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("backoffExp", "Set Backoff mode to Linear if 1 or exponential if 0 (default)",
                   UintegerValue (0),
                   MakeUintegerAccessor (&Dcf::backoffExp),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("totalDelay", "Total Delay Due to Backoff",
                   UintegerValue (0),
                   MakeUintegerAccessor (&Dcf::totalDelay),
                   MakeUintegerChecker<uint32_t> ())
