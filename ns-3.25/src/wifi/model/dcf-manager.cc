...
void
DcfState::UpdateFailedCw (uint32_t backoffExp)
{ 
  //see 802.11-2012, section 9.19.2. 
  std::string nodeType = "";
  if(backoffExp){
     nodeType = "MS (Exponential)";
  }
  else{
     nodeType = "hpMS (Linear)";
  }
  //std::cout << "nodeMode = " << nodeType  << std::endl;
  if (backoffExp == 0){
...
DcfManager::DoRestartAccessTimeoutIfNeeded (void)
{
  ...
  if (accessTimeoutNeeded)
    {
      totalDelay+= expectedBackoffEnd - Simulator::Now();
      //MY_DEBUG ("expected backoff end=" << expectedBackoffEnd);
      //std::cout << "Total Delay (ns) due to backoff for Node " << "is: " << expectedBackoffEnd - Simulator::Now() << std::endl;
      ...
      if (m_accessTimeout.IsExpired ())
        ...
        std::cout << "Total Delay for manager with tag " << ((unsigned long) this % 89) + 10 << " is: " << totalDelay.GetSeconds() << std::endl;
